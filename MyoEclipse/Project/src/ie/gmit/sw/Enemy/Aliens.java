...
import ie.gmit.sw.Sound.Sounds;
...
public class Aliens {
	...
	private Enemies aliens[][] = new Enemies[5][11]; // 2D array keeping track of each individual alien
	private Enemies ufo; // used to keep track of random UFO that shows up on top of screen
	...
	public Aliens(int wave, ScoreBoard getScore, Player getShip, Blocks getShield) {
		ship = getShip;
		score = getScore;
		shield = getShield;
		...
		for (int i = 0; i < 11; i++) { // generates 2D array of aliens of each type (except for random UFO)
			aliens[0][i] = new Enemies(1, 138 + 47 * i, 79 + waveMod);
		}
		for (int i = 0; i < 11; i++) {
			for (int j = 1; j < 3; j++) {
				aliens[j][i] = new Enemies(2, 133 + 47 * i, 79 + 34 * j + waveMod);
			}
		}
		for (int i = 0; i < 11; i++) {
			for (int j = 3; j < 5; j++) {
				aliens[j][i] = new Enemies(3, 131 + 47 * i, 79 + 34 * j + waveMod);
			}
		}
	}
	...
	public void ufoTrack() { // used to generate and move random UFO
		if (coin.nextInt(5000) == 0 && ufo == null) { // generate new UFO
			ufo = new Enemies(4, 770, 45);

			// play music
			try {
				ufoMusic = Sounds.play("ufo");
	...
	public void move() {
		if (!loseGame) { // stop moving if game over
			...
			if (globDir == LEFT) { // while aliens are going to left
				...
				if (topLeft + 47 * spaceTest - 18 > 30) { // if leftmost alien still has not hit bounds, keep moving
															// left
					topLeft -= 18;
					bottomRight -= 18;
					for (int i = 0; i < 11; i++) {
	...
	private int hEdgeCheck(int lvl, int dir) { // recursive method that returns the leftmost or rightmost "active"
												// column (one that still has aliens on it)
		if (lvl < 11 && lvl >= 0) { // limits bounds
			...
			} else { // otherwise if column is alive, this must be the left or rightmost active
						// column
				return lvl;
			}
	...
	public boolean metronome() { // function manages "beats", i.e. alien move speed, which image to display, etc.
		...
		if (beatCount % beatModifier == 0) { // move aliens when counter is divisible by beat modifier
			...
			// play music
			try {
				Sounds.play(Integer.toString(musicCount));
			} catch (UnsupportedAudioFileException e) {
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
	...
	public ArrayList<Bullet> attack() { // used to generate bullets from aliens
		boolean alreadyShot = false; // flag to determine if aliens have shot on that call, prevents aliens from
										// attacking all at once
		for (int i = 0; i < 11; i++) {
			for (int j = 4; j >= 0; j--) {
				if (aliens[j][i] != null) { // for each alien
					int ranNoGen = coin.nextInt((int) (((noEnemies + 1) / 56.0) * 100)); // random number generator more
																							// likely to hit 0 when less
																							// aliens are present
					if (ranNoGen == 0) {
						alreadyShot = true;
						int newX = aliens[j][i].getX() + (int) (aliens[j][i].getSizeX() / 2); // generate new bullet
																								// from specific alien
