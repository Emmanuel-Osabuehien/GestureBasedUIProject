...
public class Bullets {
	...
	public Bullets(Player getPlayer, Aliens getEnemies, Blocks getShield) {
		player = getPlayer;
		enemies = getEnemies;
		shield = getShield;
	}
	...
	public void trackBullets() { // tracks bullets in the game
		...
		if (playerShot != null) {
			...
			if (shieldHit) {
				playerShot = null;
				canShoot = true;
			} else if (playerShot.getY() <= 40 || enemies.collide(playerShot.getRect())) { // check if bullet hits enemy
